...
|---------|-------------|---------------|
| `getServerSideProps` | CMS pages needing live preview, paginated lists | `[slug].js`, `landing.js`, `products/index.js` |
| `getStaticProps` + ISR | Mostly static content with periodic refresh | `products/[slug].js` (`revalidate: 60`, `fallback: 'blocking'`) |
